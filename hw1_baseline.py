...
print("文档向量化耗时：%fs" % (time.time()-vectorized_st))

# 计算文档两两之间的相似度
def cosine_similarity(vector1,vector2):
    inner_product = np.sum(vector1 * vector2)
    mod_product = np.linalg.norm(vector1) * np.linalg.norm(vector2)
    
    # 有一个文档为空，那么必然不相似
    if mod_product == 0.0:
        return 0.0
    return inner_product/mod_product
...
print("计算相似度耗时：%fs" % (time.time()-sim_st))

with open('./similarity_res.txt','w') as f:
